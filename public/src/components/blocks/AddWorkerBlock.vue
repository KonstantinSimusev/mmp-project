    ...
    methods: {
        ...
        addWorker() {
            if (this.workerName != 'Выберите ФИО') {
                ...
                const addWorker = this.addWorkers.filter(worker =>
                    worker.initials == initials);

                if (addWorker.length == 0) {
                    const worker = {
                        fullname: this.workerName,
                        initials: initials,
                    }
                    this.addWorkers.push(worker);
                }
            }
        },
        ...
        createWorkerNames() {
            let optionName = {};
            const names = this.getNotTeamWorkers().map(worker => 
                worker.fullname).sort();

            this.nameOptions.push('Выберите ФИО');
            this.nameOptions.push(...names);

            this.nameOptions = this.nameOptions.map(name => 
...
<template>
    <div class="wrapper px-3 p-3 mb-5 mt-3 rounded-4 shadow-sm">
        ...
        <div class="mb-1 ps-1 text-secondary small">Добавить работника :</div>

        <select class="form-control form-select ps-3 mb-3" v-model="workerName" @change="this.addWorker()">
            <option v-for="option in this.nameOptions" :key="option.id" v-bind:value="option.value">{{ option.value }}</option>
        </select>

        <div class="worker__block small d-flex justify-content-between align-items-center mt-2 border border-secondary p-2 ps-3 rounded" v-for="(worker, number) in this.addWorkers" :key="worker">

            <div>#{{ number + 1 }} {{ worker }}</div>

            <button class="btn btn-close btn-sm" @click="this.deleteWorker(worker, this.addWorkers)"></button>
